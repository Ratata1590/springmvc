...
public class ObjectEndpoint {
	...
	public Object convertObject(@RequestHeader(required = true) Integer objectId,
			@RequestHeader(required = true) String typeName, @RequestHeader String methodName) throws Exception {
    ...
    switch (node.get(ActionObjectType.ACTIONTYPE).asInt()) {
      case ActionObjectType.CONSTRUCTOR:
    	((ObjectNode) node).remove(ActionObjectType.ACTIONTYPE);
	...
	@RequestMapping(value = "/testCompiler", method = RequestMethod.POST)
	public void testCompiler(@RequestBody String source) throws Exception{
	...
	// public Object createObject(@RequestHeader String className, @RequestBody
	// JsonNode param)
	// throws Exception {
	...
	// .getConstructor(paramTypeList.toArray(new
	// Class<?>[paramTypeList.size()]))
	// .newInstance(paramObjectList.toArray(new
	// Object[paramObjectList.size()]));
