...
public interface RObjectKeyRepo extends JpaRepository<RObjectKey, Long> {
	...
	@Query("select a.keyName from RObjectKey a")
	Set<String> getAllKeyName();

	@Query("select a from RObjectKey a where a.robject.id = :robjectId and a.keyName = :keyName")
	public RObjectKey findbyValue(@Param(value = "robjectId") Long robjectId, @Param(value = "keyName") String keyName);
}
