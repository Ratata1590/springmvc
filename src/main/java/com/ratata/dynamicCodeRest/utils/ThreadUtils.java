...
/**
 * The ThreadUtilities class provides a collection of static methods for listing and finding Thread,
 * ThreadGroup, and ThreadInfo objects.
 * <p>
...
public final class ThreadUtils {
  ...
  // Thread groups
  /**
   * The root thread group saved on the first search for it. The root group doesn't change for the
   * life of the JVM, so once found there is no need to find it again.
   */
  private static ThreadGroup rootThreadGroup = null;

  /**
   * Get the root thread group in the thread group tree. Since there is always a root thread group,
  ...
  /**
   * Get a list of all thread groups. Since there is always at least one thread group (the root),
   * this method never returns a null or empty array.
   *
  ...
  /**
   * Get the thread group with the given name. A null is returned if no such group is found. If more
   * than one group has the same name, the first one found is returned.
   *
  ...
  // Threads
  /**
   * Get a list of all threads. Since there is always at least one thread, this method never returns
  ...
  /**
   * Get a list of all threads in a thread group. An empty array is returned if there are no threads
   * in the group.
   *
  ...
  /**
   * Get a list of all threads in a named thread group. A null is returned if the group cannot be
   * found. An empty array is returned if there are no threads in the group.
   *
  ...
  /**
   * Get a list of all threads, sorted from highest to lowest priority. Since there is always at
   * least one thread, this method never returns null or an empty array. Since threads may change
   * their priority during this method's sort, the returned thread list may not be correct by the
   * time it is returned.
   *
   * @return an array of threads
   */
  ...
  /**
   * Get a list of all daemon threads. An empty array is returned if there are no daemon threads.
   *
  ...
  /**
   * Get a list of all threads with a given thread state. Thread states are defined in the
   * Thread.State enum for the Thread class. Principal thread states include RUNNABLE, WAITING,
   * TIMED_WAITING, and BLOCKED. An empty array is returned if there are no threads in the chosen
   * state.
   *
  ...
  /**
   * Get the thread with the given name. A null is returned if no such thread is found. If more than
   * one thread has the same name, the first one found is returned.
   *
  ...
  /**
   * Get the thread with the given ID. A null is returned if no such thread is found.
   *
  ...
  /**
   * Get the thread for the given thread info. A null is returned if the thread cannot be found.
   *
  ...
  // ThreadInfo
  /**
   * Get a list of all thread info objects. Since there is always at least one thread running, there
   * is always at least one thread info object. This method never returns a null or empty array.
   *
  ...
  /**
   * Get the thread info for the thread with the given name. A null is returned if no such thread
   * info is found. If more than one thread has the same name, the thread info for the first one
   * found is returned.
   *
   * @param name the thread name to search for
  ...
  /**
   * Get the thread info for the thread with the given ID. A null is returned if no such thread info
   * is found.
   *
   * @param id the thread ID to search for
   * @return the thread info, or null if not found
  ...
  /**
   * Get the thread info for the given thread. A null is returned if the thread info cannot be
   * found.
   *
  ...
  /**
   * Get the thread holding a lock on the given object. A null is returned if there is no such
   * thread.
   *
  ...
  /**
   * Get the thread who's lock is blocking the given thread. A null is returned if there is no such
   * thread.
   *
