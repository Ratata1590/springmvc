...
public class MavenRepoEndpointController {
	...
	public void uploadJarToRepo(@RequestBody(required = true) MultipartFile file,
		...
		Artifact jarArtifact = new DefaultArtifact(groupId, artifactId, null, "jar", version, null, tempFile);
	...
	public Object getMetadata(@RequestHeader(required = true) String groupId,
		...
		File metaFile = new File(localRepo.getBasedir().getPath().concat(File.separator).concat(groupId)
				.concat(File.separator).concat(artifactId).concat(File.separator).concat("maven-metadata-local.xml"));
	...
	public void createClassLoader(@RequestHeader(required = true) String classLoaderName,
		...
		if (parent != null) {
			...
			classLoaderName = mavenDependencies.get(keyParent).asText().concat(":").concat(classLoaderName);
		} else {
	...
	private List<URL> resolveSingleRepo(JsonNode node) throws Exception {
		...
		if (node.get(keyProxyURL) != null && node.get(keyProxyPort) != null && node.get(keyUsername) != null
				&& node.get(keyPassword) != null) {
			...
			Proxy proxy = new Proxy(protocol, node.get(keyProxyURL).asText(), node.get(keyProxyPort).asInt(),
