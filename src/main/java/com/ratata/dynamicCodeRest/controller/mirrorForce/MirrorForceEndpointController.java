...
public class MirrorForceEndpointController {
  ...
  private static void processConnect(JsonNode config) throws Exception {
    ...
    switch (config.get(type).asText()) {
      case oneWay:
        inputStreamList.get(config.get(in).asText())
            .connect(outputStreamList.get(config.get(out).asText()));
        break;
      case twoWay:
        inputStreamList.get(config.get(in).asText())
            .connect(outputStreamList.get(config.get(out).asText()));
        outputStreamList.get(config.get(in).asText())
            .connect(inputStreamList.get(config.get(out).asText()));
        break;
    }
  }
