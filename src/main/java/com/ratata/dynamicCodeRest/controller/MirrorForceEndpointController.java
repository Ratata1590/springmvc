...
public class MirrorForceEndpointController {
  private static Map<String, PipedInputStream> inputStreamList =
      new ConcurrentHashMap<String, PipedInputStream>();
  private static Map<String, PipedOutputStream> outputStreamList =
      new ConcurrentHashMap<String, PipedOutputStream>();
  ...
  public static String mirrorControlCreate() throws Exception {
    String tmpsessionId = getSaltString();
    while (inputStreamList.containsKey(tmpsessionId)
        || outputStreamList.containsKey(tmpsessionId)) {
  ...
  public static void mirrorControlConnect(@RequestBody JsonNode config) throws Exception {
    if (config.isArray()) {
  ...
  private static void processConnect(JsonNode config) throws Exception {
    ...
    switch (config.get(type).asText()) {
      case oneWay:
        inputStreamList.get(config.get(in)).connect(outputStreamList.get(config.get(out)));
        break;
      case twoWay:
        inputStreamList.get(config.get(in)).connect(outputStreamList.get(config.get(out)));
        outputStreamList.get(config.get(in)).connect(inputStreamList.get(config.get(out)));
        break;
    }
  }

  @RequestMapping(value = "/socketControl/disconnect", method = RequestMethod.POST)
  public static void mirrorControlDisconnect(@RequestHeader String sessionId) throws Exception {
    try {
