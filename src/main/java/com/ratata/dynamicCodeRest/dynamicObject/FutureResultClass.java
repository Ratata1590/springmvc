...
public class FutureResultClass extends Thread implements FutureResult {
  ...
  public void run() {
    ...
    try {
      result = DynamicObject.callClassMethod(
          DynamicCodeRestEndpointController.classList.get(className), methodName, param);
    } catch (Exception e) {
  ...
  public void setThreadInfo(String className, String methodName, Object[] param, Integer timeOut) {
    ...
    setName(this.className.concat(":").concat(this.methodName).concat(":")
