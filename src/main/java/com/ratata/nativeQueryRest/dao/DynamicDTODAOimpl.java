...
public class DynamicDTODAOimpl implements DynamicDTODAO {
	...
	private void processMergeArray(JsonNode tnode, JsonNode innerNode, Integer i, JsonNode data) {
		if (tnode.has(Const.PARAM_MERGEARRAY) && tnode.get(Const.PARAM_MERGEARRAY).asBoolean() && data.isArray()) {
	...
	private JsonNode processQueryLink(JsonNode node, JsonNode param) {
		try {
			return Mapper.mapper.valueToTree(coreDAO
					.processQueryObject(QueryListHolder.queryList.get(node.get(Const.LINK_QUERY).asText()), param));
		} catch (Exception e) {
