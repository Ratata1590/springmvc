...
public class NativeQueryDAOImpl implements NativeQueryDAO {
  ...
  public Object nativeQuery(String query, String className, List<String> resultSet,
      String queryMode, ArrayNode param, Boolean isNative, Integer offset, Integer limit)
      throws Exception {
  ...
  private Object returnResult(String query, String className, ArrayNode param, Integer offset,
      Integer limit, String queryMode, Boolean isNative) throws Exception {
    Query queryObj;
    if (isNative) {
  ...
  public Object processQueryObject(JsonNode queryObject, ArrayNode param) throws Exception {
    ...
    Boolean isNative = queryObject.has(Const.PARAM_ISNATIVE)
        ? queryObject.get(Const.PARAM_ISNATIVE).asBoolean() : true;
    String queryMode = queryObject.has(Const.PARAM_QUERYMODE)
        ? queryObject.get(Const.PARAM_QUERYMODE).asText() : "L";
    List<String> resultSet =
        UtilNativeQuery.arrayNodeToListString((ArrayNode) queryObject.get(Const.PARAM_RESULTSET));
    ...
    String className = queryObject.has(Const.PARAM_CLASSNAME)
        ? queryObject.get(Const.PARAM_CLASSNAME).asText() : "";
    Integer offset =
        queryObject.has(Const.PARAM_OFFSET) ? queryObject.get(Const.PARAM_OFFSET).asInt() : 0;
    Integer limit =
        queryObject.has(Const.PARAM_LIMIT) ? queryObject.get(Const.PARAM_LIMIT).asInt() : 0;
  ...
  private Object nestedNativeQuery(String query, List<String> resultSet, String queryMode,
      ArrayNode param, Boolean isNative, JsonNode insideObject) throws Exception {
    if (queryMode.equals(Const.QUERYMODE_SINGLE)) {
      Map<String, Object> rootResult =
          (Map<String, Object>) nativeQuery(query, "", resultSet, queryMode, param, isNative, 0, 0);
      return processSingleNestedNode(rootResult, insideObject);
    }
    List<Map<String, Object>> rootResultList = (List<Map<String, Object>>) nativeQuery(query, "",
        resultSet, queryMode, param, isNative, 0, 0);
  ...
  public Object processSingleNestedNode(Map<String, Object> rootResult, JsonNode insideObject)
    ...
    while (obj.hasNext()) {
      String key = obj.next();
      rootResult.put(key,
          processQueryObject(insideObject.get(key).get(Const.PARAM_SINGLEREQUEST_DATA),
              resolveParamArrayNode(rootResult,
                  (ArrayNode) insideObject.get(key).get(Const.PARAM_SINGLEREQUEST_PARAM))));
    }
    return rootResult;
  }
