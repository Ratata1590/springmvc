...
public class MavenRepoEndpointController {
  ...
  public static void createClassLoader(@RequestHeader(required = true) String classLoaderName,
    ...
    DynamicCodeRestEndpointController.classLoaderList.put(classLoaderName, classLoader);
    DynamicCodeRestEndpointController.configList.put(classLoaderName, mavenDependencies);
    System.gc();
  }
  ...
  private static ClassLoader resolveParentClassloader(String parentName) {
    ...
    return DynamicCodeRestEndpointController.classLoaderList.get(parentName);
  }
  ...
  public static void removeClassLoader(@RequestHeader String classLoaderName) throws Exception {
    for (String cl : DynamicCodeRestEndpointController.classLoaderList.keySet()) {
      String[] tree = cl.split(parent_SEP);
      for (String name : tree) {
        if (name.equals(classLoaderName)) {
          DynamicCodeRestEndpointController.classLoaderList.remove(cl);
          DynamicCodeRestEndpointController.configList.remove(cl);
          break;
        }
      }
    }
